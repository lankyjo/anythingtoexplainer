#!/usr/bin/env bash
# Full render: VER=v1 scripts/render.sh  -> renders/<slug>_v1.mp4 + fin_frames/ + renders/sheet_v1.html
set -e
...
npx remotion render build_full Video "renders/${SLUG}_${V}.mp4" --codec=h264 --crf=16 --concurrency=${CONC:-$NCPU} --timeout=${RTIMEOUT:-300000} --log=error
