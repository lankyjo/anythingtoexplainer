#!/usr/bin/env python3
"""Take one still every `step` frames from fin_frames/f_%04d.jpg and build a thumbnail-grid HTML
for QC reading. Usage: sheet.py <frames_dir> <out.html> [step=60] [cols=6]"""
...
def sent_at(n):
    ...
    for s in tl['sentences']:
        # Narration may come from a user-supplied article/excerpt; escape before embedding in HTML
...
<h3>Film thumbnails · every {step} frames · {len(files)} frames total</h3><div class="g">{''.join(cells)}</div>'''
