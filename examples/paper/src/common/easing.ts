// Easing helpers shared by all packs. Pure functions; n = frames since the element's start (N - f0).
export const clamp01 = (v: number) => Math.min(1, Math.max(0, v));

/** Slide-in progress 0->1: 1 - (1 - n/N)^p. Usage: y = yEnd + delta * (1 - slideIn(n)). */
export const slideIn = (n: number, N = 22, p = 2.5) => 1 - Math.pow(1 - clamp01(n / N), p);
/** Power ease-out "remaining" amount (1 - n/N)^p: multiply by the displacement delta. */
...
/** Power ease-in t^p (exits accelerate). If written as "delta = c * t^p with t in frames", use Math.pow(n, p). */
export const powIn = (p: number) => (t: number) => Math.pow(clamp01(t), p);
/** Power ease-in-out: t<0.5 -> 0.5*(2t)^p, else 1 - 0.5*(2-2t)^p. */
...
/** CSS cubic-bezier(x1,y1,x2,y2) evaluation (Newton with a bisection fallback), t in [0,1] -> progress. */
...
/** 21-frame scale-in curve cubic-bezier(0.10,0.10,0.35,1). Usage: scale = s0 + (s1-s0)*BEZ_SCALE_IN(n/21). */
export const BEZ_SCALE_IN = cubicBezier(0.1, 0.1, 0.35, 1);

/**
 * Emphasis scale pulse: scale 1 -> peak (1.11-1.147) -> 1 over up / hold / down frames, easeInOut,
 * no opacity change. Lines and arrows do not take part. Start ~2 frames after the subtitle block
 * start. Returns 1 for n <= 0 and after the pulse.
 */
...
/**
 * Keyframe interpolation (t is a film frame or a relative frame).
 * First-value trap: below the first key it returns the FIRST value (not 0) - a curve that only
 * starts mid-shot must begin with [shot start frame, start value], or the first half of the shot
 * sits on the first keyframe's value. Above the last key it returns the last value.
 * Optional ease applies to each segment's normalised progress (linear by default).
 */
...
/** Step hold: below the first frame returns the first value; otherwise the last key <= t (hard cuts, state machines). */
...
/** Deterministic pseudo-random [0,1) from any number of seeds. Use this, never Math.random. */
